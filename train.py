...
save_path = '.tmps4'+ '/' + opt.dataset + '_' + opt.backbone + '_'+ opt.mode

"""
Initialize experiments log
"""
logging.basicConfig(level=logging.INFO)
writer = SummaryWriter('.tmps4/log/' + opt.dataset + '_' + opt.backbone + '_'+ opt.mode + '_' + f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}')
...
## model = torch.load('./.tmps3/cdd_resnet_rsp_300/checkpoint_epoch_74.pth')


criterion = get_criterion(opt)
if opt.backbone == 'resnet':
    optimizer = torch.optim.SGD(model.parameters(), lr=0.005, momentum=0.99, weight_decay=0.0005) # 0.001 Be careful when you adjust learning rate, you can refer to the linear scaling rule
